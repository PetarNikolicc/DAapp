...
# -------------------------------------------------------------------
# Definiera save_dataset-funktionen här, högst upp i koden
...
# ----------------------------------------------------------------------------------------------------------------------------
# Modul 1: Datainsamling och avancerade dataförklaringar (utan beskrivningar)
def load_data():
    ...
    if dataset_option == "Ladda upp dataset":
        ...
        if file:
            try:
                ...
                # Visa endast dataframe head och kör statistiska tester
                st.write("### Dataframe")
                st.dataframe(data.head(50))  # Visar en större mängd data (50 rader)
...
# ----------------------------------------------------------------------------------------------------------------------------
# Modul 1: Datainsamling (utan statistiska tester)
def load_data():
    st.title("Datainsamling")
...
# ----------------------------------------------------------------------------------------------------------------------------
# Modul 3: Data Preprocessing med kontinuerlig uppdatering
def preprocessing_module(data):
    ...
    with col3:
        ...
        if st.button("Bekräfta saknade värden"):
            ...
            elif missing_option == "Fyll i saknade värden med ett specifikt värde":
                fill_value = st.number_input("Ange ett specifikt värde för att fylla i saknade värden", key="fill_value")
    ...
    with col3:
        st.subheader("Kodning av kategoriska variabler")
        cat_cols = st.multiselect("Välj kategoriska kolumner att koda", data.select_dtypes(include=['object']).columns.tolist(), key="encode_columns")
        encoding_option = st.selectbox("Välj kodningsmetod", ["Label Encoding", "One-Hot Encoding"], key="encoding_option")
        if st.button("Bekräfta kodning"):
            if len(cat_cols) > 0:
